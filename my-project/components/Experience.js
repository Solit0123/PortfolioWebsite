

function Experience() {
  return (
    <div className=" bg-blue-50">

    
      <div className="container mx-auto flex flex-col text-center space-y-4 py-16">
          <h2 className="text-5xl font-semibold ">Experience</h2>
          <p className="text-md">Check out my LinkedIn for my previous experience </p>
          <a className="" href="https://www.linkedin.com/in/jose-santos-solorio-315a811b7/" target="_blank">
              <img src="./linkedin.svg" width="45px" height="45px" alt="" className="mx-auto hover:scale-105 hover:shadow-lg transition duration-150 ease-out" />
              </a>
        
      </div>
    </div>
  )
}

export default Experience